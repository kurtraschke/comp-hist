...
struct Node *comp_str (char *s1, char*s2, struct Node *node)
{
        ...
        if (*s1>*s2) return (node->right); else return (node->left);
}
...
struct Node *seek_node (struct Node *nodes, char *s)
{
	...
	while (nodes && (0 != (diff=strcmp (s, nodes->id))))
              if (diff>0) nodes=seek_node(nodes->right,s);
              else nodes=seek_node(nodes->left,s);
...
static struct Node *add_new_node (struct Node **tree)
{
...
#ifndef SPEED
   if (seek_node (*tree, tmp->id))
   	error (0, NULL, "Duplicate node id");
#endif

   tmp->left = NULL;
   tmp->right= NULL;
   *tree=insert_node(*tree,tmp);

   /* Clear all field contents */
...
void create_one_link(struct Node *t, struct Node *node_tree)
	...
	/* If field is required make sure it has a value */
	        ...
	        if ((fields[fn].flags & REQUIRED) && (t->field[fn] == NULL))
		{
		 ...
		 if ((fields[fn].typ == LINK) && t->field[fn])
		 {
			...
			while (nl)
			{
				if (NULL == (tmp = seek_node (node_tree,
					(char *)(nl->node))))
				{
...
void create_all_links(struct Node *tree, struct Node *nodetree)
{
        if (tree!=NULL)
        {
                create_one_link(tree,nodetree);

                create_all_links(tree->left,nodetree);
                create_all_links(tree->right,nodetree);
        }
}
...
void disp_all_nodes(struct Node *nodetree)
{
        if (nodetree!=NULL)
        {
                disp_one_node(nodetree);

                disp_all_nodes(nodetree->left);
                disp_all_nodes(nodetree->right);
        }
};
